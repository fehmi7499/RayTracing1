...
private:
    Ray AdjustRay(int row, int column, Surface* surface, bool modify, Ray reflectionRay, SceneParser* sceneData);
    ...
    float ComputeShadowFactor(const Ray& incomingRay, Light* illumination, SceneParser* sceneData);
    Ray ApplyRefractionSL(const Ray& incomingRay, const vec3& surfaceNormal, const vec3& rayPath, float refractiveIndex);
    vec4 CalculatePixelColor(int row, int column, Ray incomingRay, int recursionDepth, SceneParser* sceneData);
